...
class Schedule_wave(db.Document):
    ...
    start_time = db.DateTimeField( default=datetime.now() - timedelta(hours=5) )
    end_time = db.DateTimeField( default=datetime.now() - timedelta(hours=5) )
    status = db.BooleanField( default=True )
    
class Activity(db.Document):
    act_id = db.IntField( unique=True )
    vin = db.StringField()
    scan_time = db.DateTimeField( default=datetime.now() - timedelta(hours=5) )
